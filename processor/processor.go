...
func Process() {
	...
	// Less than or 0 threads makes no sence so set to at least one
	if NoThreads <= 0 {
	...
	fileListQueue := make(chan *FileJob, NoThreads*1000)           // Files ready to be read from disk
	fileReadJobQueue := make(chan *FileJob, NoThreads*10)          // Workers reading from disk
	fileReadContentJobQueue := make(chan *FileJob, NoThreads*1000) // Files ready to be processed
	fileProcessJobQueue := make(chan *FileJob, NoThreads)          // Workers doing the hard work
	fileSummaryJobQueue := make(chan *FileJob, NoThreads*1000)     // Files ready to be summerised
