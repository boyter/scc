...
func commentState(sta *state) (int, int64, [][]byte) {
	for i := sta.index; i < sta.endPoint; i++ {
		curByte := sta.fileJob.Content[i]
		sta.index = i

		if curByte == '\n' {
			return i, sta.currentState, sta.endComments
		}
		...
		if checkForMatchSingle(curByte, sta.index, sta.endPoint, sta.endComments[len(sta.endComments)-1], sta.fileJob) {
			...
			if len(sta.endComments) == 0 {
				...
				if sta.currentState == SMulticommentCode {
					sta.currentState = SCode // TODO pointless to change here, just set S_MULTICOMMENT_BLANK
				} else {
					sta.currentState = SMulticommentBlank
				}
			}
	...
	return sta.index, sta.currentState, sta.endComments
}

func blankState(sta *state) (int, int64, [][]byte, bool) {
	...
	case TString:
		...
		sta.index = index
		sta.ignoreEscape = ignoreEscape
		return sta.index, sta.currentState, sta.endComments, ignoreEscape
	...
	for index := checkBomSkip(sta.fileJob); index < len(sta.fileJob.Content); index++ {
		...
		if !isWhitespace(sta.fileJob.Content[sta.index]) {
			...
			case SMulticomment, SMulticommentCode:
				index, currentState, endComments = commentState(sta)
			case SBlank, SMulticommentBlank:
				...
				index, currentState, endComments, ignoreEscape = blankState(sta)
			}
		}
		...
		if sta.fileJob.Content[sta.index] == '\n' || sta.index >= sta.endPoint {
			...
			case SCode, SString, SCommentCode, SMulticommentCode:
				...
				if Trace {
					printTrace(fmt.Sprintf("%s line %d ended with state: %d: counted as code", sta.fileJob.Location, sta.fileJob.Lines, sta.currentState))
				}
			case SComment, SMulticomment, SMulticommentBlank:
				...
				if Trace {
					printTrace(fmt.Sprintf("%s line %d ended with state: %d: counted as comment", sta.fileJob.Location, sta.fileJob.Lines, sta.currentState))
				}
			case SBlank:
				sta.fileJob.Blank++
				if sta.fileJob.Callback != nil {
				...
				if Trace {
					printTrace(fmt.Sprintf("%s line %d ended with state: %d: counted as blank", sta.fileJob.Location, sta.fileJob.Lines, sta.currentState))
				}
			}
		}
	}
