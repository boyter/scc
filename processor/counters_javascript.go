...
// jsAnchorBit gives each anchor byte of JavaScript a bit; jsAnchorPrev[prev]
// holds the bits of every anchor whose first test could still pass with prev in
// front of it. The AND of the two is zero exactly where jsComplexityAnchored
...
//
// Measured over 21,467 files of JavaScript, 140MB reaching the code scan: it
// stops on 13.6% of the bytes it looks at, 71% of those stops are complexity
// anchors, and 93% of the anchor stops count nothing. That is 9.0% of every
// byte the code loop reads, spent reaching a call that says no. The byte in
// front settles three quarters of them.
//
// The postfix ? checks and the two comment forms are not here, having switch
// arms of their own: only the default arm reaches this table.
const (
	jsAnchorBitF uint8 = 1 << iota
	jsAnchorBitL
	jsAnchorBitW
	jsAnchorBitEq
	jsAnchorBitC
...
var jsAnchorBit = buildJsAnchorBit()

var jsAnchorPrev = buildJsAnchorPrev()

func buildJsAnchorBit() [256]uint8 {
	var table [256]uint8
	table['f'] = jsAnchorBitF
	table['l'] = jsAnchorBitL
	table['w'] = jsAnchorBitW
	table['='] = jsAnchorBitEq
	table['c'] = jsAnchorBitC
	table['&'] = jsAnchorBitAmp
	table['|'] = jsAnchorBitPipe

	return table
}

func buildJsAnchorPrev() [256]uint8 {
	var table [256]uint8
	for value := range 256 {
		previous := byte(value)
		...
		if boundary {
			mask |= jsAnchorBitC | jsAnchorBitAmp | jsAnchorBitPipe
		}
		table[value] = mask
	}
...
func jsCodeState(content []byte, tally *counterTally, index, endPoint, floor int, stop *[256]bool) (int, counterState, []byte) {
	...
	for i := index; i < endPoint; i++ {
		...
		default:
			// Ninety three percent of anchor stops fail, and three quarters of
			...
			// jsComplexityAnchored rejects on its own first test, so it cannot
			// change a count. See jsAnchorPrev.
			if jsAnchorPrev[byteBefore(content, i, floor)]&jsAnchorBit[curByte] == 0 {
				continue
			}
