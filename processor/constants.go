...
var languageDatabase = map[string]Language{
	...
	"C++ Header": {
		...
		Keywords:        []string{},
		Heuristics: []string{
			"(?m)^\\s*#\\s*include <(cstdint|string|vector|map|list|array|bitset|queue|stack|forward_list|unordered_map|unordered_set|(i|o|io)stream)>",
			"(?m)^\\s*template\\s*<",
			"(?m)^[ \\t]*(try|constexpr)",
			"(?m)^[ \\t]*catch\\s*\\(",
			"(?m)^[ \\t]*(class|(using[ \\t]+)?namespace)\\s+\\w+",
	...
	"Objective C": {
		...
		Heuristics: []string{
			"(?m)^\\s*(@(interface|class|protocol|property|end|synchronised|selector|implementation)\\b|#import\\s+.+\\.h[\">])",
		},
		FileNames: []string{},
		SheBangs:  []string{},
	},
