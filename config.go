...
func registerFlags(flags *pflag.FlagSet, b *flagBindings) {
	...
	flags.BoolVar(boolVar(&processor.SpecialisedCounters), "exp-per-language-counters", false, "experimental, may change or be removed: count with the scanner written for the file's language where there is one, currently C, C Header, Java and JavaScript")
	...
	flags.IntVar(intVar(&processor.HistoryBuckets), "buckets", 60, "time-bucket resolution for the git timeline reports")
